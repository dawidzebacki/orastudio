...
- [ ] Krok 24 — **Kropka aktywnej strony w Nav + strona 404** — DO AKCEPTACJI. Inspiracja: navbar-aparat (skeuomorfizm odrzucony jako sprzeczny z marką; przejęto dwa motywy). (1) `Nav.astro` — aktywny link: zamiast podkreślenia **bursztynowa kropka-wskaźnik** (`::after`, `--c-amber`, 4px; desktop = wycentrowana pod etykietą jak dioda REC, mobile-panel = inline za etykietą jak kropka „ora."). Semantyka dalej przez `aria-current`. (2) **`/404`** (`src/pages/404.astro`, Netlify serwuje `dist/404.html` automatycznie) — komunikat wyśrodkowany w stylu `/dziekujemy` (eyebrow „Błąd 404" · H1 „Nie ma takiego kadru." z bursztynową kropką · lede z mailem · 2 przyciski) + **aparat budowany w HTML/CSS** (po dwóch iteracjach: line-art SVG → SVG z wypełnieniami → finalnie skeuomorficzny dalmierz z divów/gradientów wzorem codepenowej inspiracji Dawida). Wszystko na tokenach: „metal" = krem↔piasek (mieszanki `color-mix`, w ciemni odwraca się sam — ciemny korpus), partie „czarne" (szkło/uchwyty/wizjer/flesz) = `--c-shadow`+kawa (ciemne w OBU motywach), światła bevelu = `--c-bevel-hi`, czerwony guzik oryginału = **bursztynowy spust** pulsujący wolno (3.6s), grawer „ORA" w `--c-label`. Ruch wolny/miękki zamiast efekciarstwa oryginału: kołysanie ±1.2° 9s, flesz „oddycha" 7s (nie stroboskop) — wszystko off przy `prefers-reduced-motion`. Zweryfikowane zrzutami headless Edge w obu motywach. Ilustracja wydzielona do **komponentu `Camera.astro`** (klasy `cam__*`) i użyta na `/404` + **`/dziekujemy`** („klatka zrobiona" nad ✓ Wiadomość wysłana) + pokazana na `/styleguide` (sekcja XIV). ZASADA: Camera tylko na stronach niskiego ryzyka, NIE na podstronach ofertowych. ⚠️ Nauczka z dev: vite potrafi trzymać przestarzały cache modułu stylów strony (styl „znika" TYLKO na dev, build OK) — ratunek: dotknąć plik lub zrestartować `npm run dev`. Motyw aparatu dopuszczony ŚWIADOMIE tylko na 404 (strona-mrugnięcie). `noindex` + `/404` dodane do filtra sitemap (`astro.config.mjs`); zweryfikowane: brak w `sitemap-0.xml`. Brand-guardian: zero naruszeń twardych. **Uwagi miękkie (do decyzji, nie błędy):** na 404 są 2 bursztyny (spust + kropka H1) — jeśli Ada chce jednej dominanty, zdjąć puls lub przemalować spust na `--c-ink-45`; lede 404 używa pauzy „—" (treść funkcjonalna spoza copy Ady, jak `/dziekujemy`).
...
`src/components/`: **Gallery** (passe-partout, sygnaturowy; propy `href` = podpis→case study, `zoom` = powiększenie) · **Hero** · **CTAList** · **Cards** · **Process** · **Faq** · **Form** · **LookSwitcher** (opcjonalny, Śluby) · **Lightbox** (globalny `<dialog>`, powiększenie kadrów, prev/next + klawiatura, w `BaseLayout`) · **Camera** (skeuomorficzny dalmierz HTML/CSS, Krok 24 — TYLKO strony niskiego ryzyka: `/404`, `/dziekujemy`; zawsze `aria-hidden`). Jeden plik na typ, reużywalne, na tokenach.
