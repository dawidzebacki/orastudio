...
- Marka chrome: Nav sticky (tło kremowe), Footer na piasku. Logo = `wordmark-ink.svg` (krzywe, z falą) przez `Logo.astro`. Social w stopce = ikony (`SocialLinks.astro`, dane w `site.ts → socials`).
